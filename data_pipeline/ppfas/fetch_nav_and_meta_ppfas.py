...
def update_metadata(cur, scheme_code, meta):
    """Fill empty columns in mf_scheme with metadata from API.
       Also overwrite placeholder '-' for isin_growth."""
    cur.execute(
    ...
    isin_missing = (not current_isin or current_isin == '-' or current_isin.strip() == '')
    if isin_missing and meta.get("isin_growth"):
